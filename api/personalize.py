...
async def handle_personalize_request(body_text: str, auth_header: str):
    """Async handler for personalization logic."""
...
def handler(request):
    """Handle personalization requests (Vercel serverless function)."""
    ...
    try:
        ...
        try:
            response_body, status, headers = loop.run_until_complete(
                handle_personalize_request(body_text, auth_header)
            )

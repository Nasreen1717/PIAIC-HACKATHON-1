...
async def handle_signin_request(body_text: str):
    """Async handler for signin logic."""
...
def handler(request):
    """Handle signin requests (Vercel serverless function)."""
    ...
    try:
        ...
        try:
            response_body, status, headers = loop.run_until_complete(
                handle_signin_request(body_text)
            )

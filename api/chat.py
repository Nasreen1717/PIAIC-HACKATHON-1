...
async def handle_chat_request(body_text: str, auth_header: str):
    """Async handler for chat logic."""
...
def handler(request):
    """Handle chat requests (Vercel serverless function)."""
    ...
    try:
        ...
        try:
            response_body, status, headers = loop.run_until_complete(
                handle_chat_request(body_text, auth_header)
            )

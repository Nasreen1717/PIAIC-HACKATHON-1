...
async def handle_signup_request(body_text: str):
    """Async handler for signup logic."""
...
def handler(request):
    """Handle signup requests (Vercel serverless function)."""
    ...
    try:
        ...
        try:
            response_body, status, headers = loop.run_until_complete(
                handle_signup_request(body_text)
            )
